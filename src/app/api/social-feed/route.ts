...
import { fetchTwitterProfile, type TwitterProfile } from "@/lib/apify-twitter";
...
type SocialProfile = TwitterProfile;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const settled = await Promise.allSettled(
      accounts.map((a) => fetchTwitterProfile(a.handle, POSTS_PER_ACCOUNT, token))
    );
